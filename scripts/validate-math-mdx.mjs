...
for (const file of files) {
  try {
    await compile(stripFrontmatter(await readFile(file, 'utf8')), {
      format: 'mdx',
    ...
    report.push(error?.reason ?? error?.message ?? String(error));
  }
}
report.push(`MDX_VALIDATION files=${files.length} failures=${failures}`);
await writeFile('mdx-validation.txt', report.join('\n') + '\n');
